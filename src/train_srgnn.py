...
def main(args):
    ...
    model = SRGNN(hidden_size=args.hidden_dim, n_node=len(product2token), num_layers=args.n_layers).to(args.device)
            ...
            if step_count % eval_steps == 0:
                ...
                with torch.no_grad():  # Disable gradient computation for evaluation
                    for val_batch in tqdm(val_dl, desc=f"Validating (step {step_count})"):
                        x_val = val_batch["graph"].to(args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default="results/srgnn_baseline/")
